...
# Función que muestra la imagen y permite su cierre dandole a cualquier tecla
def popup(titulo, imagen):
  ...
  while True:
    if cv2.waitKey(10) > 0:
      ...
      break
# Ejemplo de uso -> mariposa = cv2.imread('mariposa.jpg') -> popup('Mariposa', mariposa)


# Función para mostrar imágenes incrustadas en ficheros y que el color sea correcto
def plot(image):
...
# Ejemplo de uso -> mariposa = cv2.imread('mariposa.jpg') -> plot(mariposa)

# Función para obtener el mejor backend para acceso a la cámara
...
# Función para ejecutar mostrar las imágenes de la cámara
def playWebcam(titulo, webcam, proceso=None):
  ...
  cv2.destroyWindow(titulo)
# Ejemplo de uso -> webcam = cv2.VideoCapture(myCam, bestCap) -> playWebcam('CAMARA', webcam) -> webcam.release()
